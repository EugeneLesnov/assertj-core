...
class Paths_assertHasDigest_with_MessageDigest_and_String_Test extends PathsBaseTest {
  ...
  @Test
  void should_fail_if_digest_is_null() throws IOException {
    // GIVEN
  ...
  void should_fail_on_unix_if_actual_is_not_readable() throws Exception {
    // GIVEN
    Path actual = Files.createFile(tempDir.resolve("actual"));
  ...
  void should_pass_if_actual_has_expected_digest() throws Exception {
    ...
    Path actual = Files.write(tempDir.resolve("actual"), "Content".getBytes());
    MessageDigest digest = MessageDigest.getInstance("MD5");
    String expected = toHex(digest.digest(Files.readAllBytes(actual)));
    // WHEN/THEN
